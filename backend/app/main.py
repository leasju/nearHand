...
def _is_valid_phone(phone: str) -> dict:
    digits = _digits_only(phone)

    if not digits:
        return {"valid": False, "message": "Digite um número de telefone"}

    if len(digits) != 11:
        return {"valid": False, "message": "Telefone deve ter 11 dígitos"}

    if digits[0] not in {"1", "2", "3", "4", "5", "6", "7", "8", "9"}:
    ...
    if area_code not in valid_area_codes:
        return {"valid": False, "message": "Código de área não existe"}
...
def validate_phone(payload: PhoneValidation):
    """Validate phone number format and Brazilian area code."""
    return _is_valid_phone(payload.telefone)
